...
var gradesDao = require('university/dao/gradesDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
		if (id) {
			...
			var gradesCount = gradesDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', gradesCount);
		} else if (metadata !== null) {
			var gradesMetadata = gradesDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(gradesMetadata, null, 2));
		} else {
			var grades = gradesDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(grades, null, 2));
		}
	}
}
...
function handlePostRequest(httpRequest, httpResponse) {
	...
	var id = gradesDao.create(entity);
