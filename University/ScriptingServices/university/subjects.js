...
var subjectsDao = require('university/dao/subjectsDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
		if (id) {
			...
			var subjectsCount = subjectsDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', subjectsCount);
		} else if (metadata !== null) {
			var subjectsMetadata = subjectsDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(subjectsMetadata, null, 2));
		} else {
			var subjects = subjectsDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(subjects, null, 2));
		}
	}
}
...
function handlePostRequest(httpRequest, httpResponse) {
	...
	var id = subjectsDao.create(entity);
...
function handlePutRequest(httpRequest, httpResponse) {
	...
	if (id !== null) {
		if (subjectsDao.get(id) !== null) {
			subjectsDao.update(entity);
