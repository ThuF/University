...
var studentsDao = require('university/dao/studentsDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
		if (id) {
			...
			var studentsCount = studentsDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', studentsCount);
		} else if (metadata !== null) {
			var studentsMetadata = studentsDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(studentsMetadata, null, 2));
		} else {
			var students = studentsDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(students, null, 2));
		}
	}
}
...
function handlePostRequest(httpRequest, httpResponse) {
	...
	var id = studentsDao.create(entity);
